...
export function GoogleSignInButton() {
  ...
  async function handleClick() {
    ...
    if (error) {
      setLoading(false);
      console.error("[google-signin]", error.message);
    }
  }
  ...
  return (
    <button
      ...
      onClick={handleClick}
      disabled={loading}
      className="flex w-full items-center justify-center gap-3 rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-900 shadow-sm transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-50 dark:hover:bg-slate-800"
    >
      <GoogleLogo />
